...
// opens that frame. Degrades gracefully: uses the video `duration` when present,
// otherwise falls back to the largest timestamp; renders nothing without usable
// timestamps.
...
export default function SelectionTimeline({
  ...
  if (frames.length === 0 || stamps.length === 0) return null;

  const duration = frames.find((f) => f.duration && f.duration > 0)?.duration ?? 0;
  const axisEnd = duration > 0 ? duration : Math.max(...stamps) * 1.05 || 1;
